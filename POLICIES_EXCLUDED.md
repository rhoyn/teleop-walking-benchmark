...
| `grove` | 0 | Its one checkpoint is NVIDIA's WBC-AGILE velocity export, vendored unchanged and byte-identical to `policies/wbc_agile_velocity/model.onnx`, already in the table under that name. grove-g1 adds only a ros2_control stack the harness does not run. |
